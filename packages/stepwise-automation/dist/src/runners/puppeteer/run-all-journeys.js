 ...
 */
async function runHook(hookPath, hookName) {
    try {
        console.log(`🔧 Running ${hookName}...`);
        const hookModule = await Promise.resolve(`${hookPath}`).then(s => __importStar(require(s)));
 ...
 */
function printSummary(results, totalJourneys) {
...
async function main() {
    ...
    const allJourneys = await (0, journey_discovery_1.discoverJourneys)(config);
    ...
    if (requestedJourneys.length > 0) {
        ...
        const notFound = requestedJourneys.filter((name) => !allJourneys.some((j) => j.id === name));
...
//# sourceMappingURL=run-all-journeys.js.map