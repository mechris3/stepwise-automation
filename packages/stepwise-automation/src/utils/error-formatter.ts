 ...
 */
function extractMethodInfo(stackLine: string): string {
 ...
 */
export function formatTestError(error: Error, context?: ErrorContext): string {
  const lines = error.stack?.split('\n') || [];

  // Extract only application code (filter out node_modules and internal Node.js)
  const appCodeLines = lines.filter(
    line =>
  ...
  const pageObjectMatch = appCodeLines.find(
    line => line.includes('.page.ts') || line.includes('Page.'),
  );
  const journeyMatch = appCodeLines.find(
    line => line.includes('.journey.ts') || line.includes('Journey.'),
  );
  ...
  // Context
  if (context?.journeyName) sections.push(`Journey: ${context.journeyName}`);
  if (context?.timestamp) sections.push(`Time: ${context.timestamp}`);
  if (context?.duration) sections.push(`Duration: ${context.duration}ms`);
  if (context?.journeyName || context?.timestamp || context?.duration) sections.push('');
  ...
  if (pageObjectMatch) sections.push(`  Page Object: ${extractMethodInfo(pageObjectMatch)}`);
  if (journeyMatch) sections.push(`  Journey Step: ${extractMethodInfo(journeyMatch)}`);
