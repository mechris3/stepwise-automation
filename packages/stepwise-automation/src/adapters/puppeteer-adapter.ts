...
import {
  ...
  selectByIndex as selectByIndexUtil,
  selectByValue as selectByValueUtil,
  selectByText as selectByTextUtil,
...
export class PuppeteerAdapter extends BaseAdapter {
  ...
  async isDisabled(selector: string): Promise<boolean> {
    ...
    await this.page.waitForSelector(selector, { visible: true, timeout: 10000 });
    const disabled = await this.page.$eval(selector, (el: any) => el.disabled);
    this.logActionComplete();
    await this.addSlowModeDelay();
    return disabled;
  }
  ...
  async getText(selector: string): Promise<string> {
    ...
    await this.page.waitForSelector(selector, { visible: true, timeout: 10000 });
    const text = await this.page.$eval(selector, (el: any) => el.textContent || '');
    this.logActionComplete();
    await this.addSlowModeDelay();
    return text;
  }
  ...
  async selectByIndex(selector: string, index: number): Promise<void> {
    ...
    await selectByIndexUtil(this.page, selector, index);
    this.logActionComplete();
    await this.addSlowModeDelay();
  }
  ...
  async selectByValue(selector: string, value: string): Promise<void> {
    ...
    await selectByValueUtil(this.page, selector, value);
    this.logActionComplete();
    await this.addSlowModeDelay();
  }
  ...
  async selectByText(selector: string, text: string, exact: boolean = false): Promise<void> {
    ...
    await selectByTextUtil(this.page, selector, text, exact);
    this.logActionComplete();
    await this.addSlowModeDelay();
  }
  ...
  async clickAndDownload(selector: string): Promise<DownloadResult> {
    ...
    while (Date.now() - start < timeout) {
      ...
      newFile = after.find((f: string) => !before.has(f) && !f.endsWith('.crdownload'));
      if (newFile) break;
      await new Promise(r => setTimeout(r, 200));
    }

    if (!newFile) throw new Error('Download timed out after 30 seconds');
